import {
  ...
  sequenceNodeType,
  stripErrorMargin
} from "./chunk-GRJX2HTI.js";
import {
  SpreadsheetSkeleton,
  hasCJKText,
  precisionTo
} from "./chunk-RWNGK3AZ.js";
import {
...
} from "./chunk-ZFT4KQ2E.js";
import {
  merge_default
} from "./chunk-OJC5CULG.js";
...
function getCellType(styles, newVal, oldVal) {
  ...
  if (hasNumberFormat(newStyle)) {
    var _newStyle$n;
    if (isTextFormat(newStyle === null || newStyle === void 0 || (_newStyle$n = newStyle.n) === null || _newStyle$n === void 0 ? void 0 : _newStyle$n.pattern))
      return CellValueType.STRING;
    return checkCellValueTypeByValue(newVal, oldVal);
  }
  if (isTextFormat(oldStyle === null || oldStyle === void 0 || (_oldStyle$n2 = oldStyle.n) === null || _oldStyle$n2 === void 0 ? void 0 : _oldStyle$n2.pattern))
    return CellValueType.STRING;
  return checkCellValueTypeByValue(newVal, oldVal);
}
...
function getSheetCommandTargetWorkbook(univerInstanceService, params) {
  const { unitId } = params;
  const workbook = unitId ? univerInstanceService.getUnit(unitId, UniverType.UNIVER_SHEET) : univerInstanceService.getCurrentUnitOfType(UniverType.UNIVER_SHEET);
  if (!workbook)
    return null;
  return {
    workbook,
...
function getSheetMutationTarget(univerInstanceService, params) {
  const { unitId, subUnitId } = params;
  const workbook = univerInstanceService.getUnit(unitId, UniverType.UNIVER_SHEET);
  if (!workbook)
    return null;
...
var AddMergeUndoMutationFactory = (accessor, params) => {
  if (!getSheetMutationTarget(accessor.get(IUniverInstanceService), params))
...
var InterceptCellContentPriority = function(InterceptCellContentPriority2) {
  InterceptCellContentPriority2[InterceptCellContentPriority2["DATA_VALIDATION"] = 9] = "DATA_VALIDATION";
  InterceptCellContentPriority2[InterceptCellContentPriority2["NUMFMT"] = 10] = "NUMFMT";
  InterceptCellContentPriority2[InterceptCellContentPriority2["CELL_IMAGE"] = 11] = "CELL_IMAGE";
  return InterceptCellContentPriority2;
}({});
...
var SheetInterceptorService = class SheetInterceptorService2 extends Disposable {
  /** @ignore */
  constructor(_univerInstanceService) {
    ...
    this.disposeWithMe(this._univerInstanceService.getTypeOfUnitAdded$(UniverType.UNIVER_SHEET).subscribe((event) => {
      this._interceptWorkbook(event.unit);
    }));
    this.disposeWithMe(this._univerInstanceService.getTypeOfUnitDisposed$(UniverType.UNIVER_SHEET).subscribe((workbook) => this._disposeWorkbookInterceptor(workbook)));
  ...
  */
  ...
  fetchThroughInterceptors(name2, effect, _key, filter2) {
    const byNamesKey = effect === void 0 ? name2 : `${name2}-${effect}`;
    ...
    if (!composed) {
      ...
      if (interceptors && filter2)
        interceptors = interceptors.filter(filter2);
  ...
  _interceptWorkbook(workbook) {
    ...
    const interceptViewModel = (worksheet) => {
      ...
      worksheet.__interceptViewModel((viewModel) => {
        ...
        sheetDisposables.add(viewModel.registerCellContentInterceptor({ getCell(row, col, effect, key, filter2) {
          ...
          return sheetInterceptorService.fetchThroughInterceptors(INTERCEPTOR_POINT.CELL_CONTENT, effect, key, filter2)(rawData, context);
        } }));
...
var SheetRangeThemeModel = class SheetRangeThemeModel2 extends Disposable {
  ...
  getDefaultRangeThemeStyle(name2) {
    return this._defaultRangeThemeMap.get(name2);
  }
  getCustomRangeThemeStyle(unitId, name2) {
    return this._ensureRangeThemeStyleMap(unitId).get(name2);
  }
  _getSheetRowVisibleFuncSet(unitId, subUnitId) {
    if (!this._rowVisibleFuncSet.has(unitId))
      this._rowVisibleFuncSet.set(unitId, /* @__PURE__ */ new Map());
    const subUnitMap = this._rowVisibleFuncSet.get(unitId);
    if (!subUnitMap.has(subUnitId))
      subUnitMap.set(subUnitId, /* @__PURE__ */ new Set());
      ...
      if (sheet) {
        ...
        for (let i = 1; i <= rowCount; i++)
          if (!sheet.getRowVisible(i))
            set.add(i);
          else if (rowManager.getRowHeight(i) === 0)
            set.add(i);
      }
    }
  }
  _ensureZebraCrossingCache(unitId, subUnitId, id) {
    if (!this._zebraCrossingCacheMap.has(unitId))
      this._zebraCrossingCacheMap.set(unitId, /* @__PURE__ */ new Map());
    const subUnitMap = this._zebraCrossingCacheMap.get(unitId);
    if (!subUnitMap.has(subUnitId))
      subUnitMap.set(subUnitId, /* @__PURE__ */ new Map());
  ...
  refreshZebraCrossingCacheBySheet(unitId, subUnitId) {
    ...
    if (cacheMap)
      cacheMap.forEach((zebraCache, id) => {
        ...
        if (themeRule)
          zebraCache.refresh(themeRule.rangeInfo.range, (row) => {
    ...
    for (let i = 0; i < themes.length; i++) {
      ...
      if (themeRule && themeRule.themeName === themeName) {
        ...
        if (zebraCacheMap) {
          ...
          if (subUnitCacheMap)
            subUnitCacheMap.delete(themes[i]);
        }
        break;
      }
    }
  ...
  */
  unregisterRangeThemeStyle(unitId, name2) {
    this._ensureRangeThemeStyleMap(unitId).delete(name2);
    this._rangeThemeMapChanged$.next({
      type: "remove",
      styleName: name2
    });
  ...
  */
  ...
  getRangeThemeStyle(unitId, name2) {
    if (this._defaultRangeThemeMap.has(name2))
      return this._defaultRangeThemeMap.get(name2);
    return this._ensureRangeThemeStyleMap(unitId).get(name2);
  }
...
var SetWorksheetRangeThemeStyleMutation = {
  ...
  type: CommandType.MUTATION,
  handler: (accessor, params) => {
    if (!getSheetCommandTarget(accessor.get(IUniverInstanceService), params))
      return false;
...
var MoveRangeMutation = {
  ...
  handler: (accessor, params) => {
    ...
    if (!from || !to)
      return false;
    const workbook = accessor.get(IUniverInstanceService).getCurrentUnitOfType(UniverType.UNIVER_SHEET);
    if (!workbook)
      return false;
...
function handleStyle(styles, oldVal, newVal) {
  ...
  if (merge2) {
    Tools.removeNull(merge2);
    Object.entries(merge2).forEach(([key, val]) => {
  ...
  if (Tools.isEmptyObject(merge2))
    ...
    oldVal.s = styles.setValue(merge2);
  const newValueStream = newVal.v ? `${newVal.v}\r
` : "";
  if (!newVal.p && oldVal.p) {
    var _oldVal$p$body;
    if (newValueStream && newValueStream !== ((_oldVal$p$body = oldVal.p.body) === null || _oldVal$p$body === void 0 ? void 0 : _oldVal$p$body.dataStream))
      delete oldVal.p;
    else
...
function transformStyle(oldStyle, newStyle) {
  ...
  for (const key of STYLE_KEYS) {
    ...
    if (sanitizedValue === void 0)
      continue;
    if (key === "bd")
      backupStyle[key] = transformBorders(backupStyle[key] || {}, sanitizedValue);
...
function transformBorders(oldBorders, newBorders) {
  ...
  for (const key of BORDER_KEYS) {
    if (!Object.prototype.hasOwnProperty.call(newBorders, key))
      continue;
...
function mergeStyle(oldStyle, newStyle, isRichText = false) {
  ...
  for (const key of STYLE_KEYS) {
    ...
    if (sanitizedValue === void 0)
      continue;
    if (key === "bd")
      backupStyle[key] = mergeBorderData(backupStyle[key], sanitizedValue);
    else
...
function mergeRichTextStyle(p, newStyle) {
  ...
  for (const textRun of p.body.textRuns) {
    ...
    if (index < st) {
      ...
      const merge3 = mergeStyle({}, newStyle, true);
      merge3 && Tools.removeNull(merge3);
      if (!Tools.isEmptyObject(merge3))
        tr.ts = merge3;
      newTextRuns.push(tr);
    }
    const merge2 = mergeStyle(ts, newStyle, true);
...
var overwriteCellPropertiesSet = /* @__PURE__ */ new Set([
  "f",
  "p",
  "si",
  "custom",
...
var SetWorksheetColWidthMutation = {
  ...
  handler: (accessor, params) => {
    ...
    for (let i = 0; i < ranges.length; i++) {
      const range = ranges[i];
      for (let j = range.startColumn; j < range.endColumn + 1; j++) {
        if (!worksheet.getColVisible(j))
          continue;
        if (typeof params.colWidth === "number")
          manager.setColumnWidth(j, params.colWidth);
        else if (Tools.isDefine(params.colWidth[j]))
          manager.setColumnWidth(j, params.colWidth[j]);
      }
    }
    return true;
  }
};
var SetWorksheetColumnCountUndoMutationFactory = (accessor, params) => {
  const target = getSheetMutationTarget(accessor.get(IUniverInstanceService), params);
  if (!target)
...
var SetWorksheetColumnCountMutation = {
...
var SetWorksheetRowCountUndoMutationFactory = (accessor, params) => {
  const target = getSheetMutationTarget(accessor.get(IUniverInstanceService), params);
  if (!target)
...
var SetWorksheetRowCountMutation = {
...
var SetWorksheetRowAutoHeightMutationFactory = (params, worksheet) => {
    ...
    for (const { startRow, endRow } of ranges)
      for (let rowIndex = startRow; rowIndex <= endRow; rowIndex++)
        if (typeof rowHeight === "number")
          ...
          manager.setRowHeight(rowIndex, rowHeight[rowIndex]);
    return true;
  }
};
...
var SetWorksheetActiveOperation = {
  ...
  handler: (accessor, params) => {
    const workbook = accessor.get(IUniverInstanceService).getUnit(params.unitId, UniverType.UNIVER_SHEET);
    if (!workbook)
      return false;
...
var SheetSkeletonChangeType = function(SheetSkeletonChangeType2) {
  SheetSkeletonChangeType2["SET_WORKSHEET_ROW_HEIGHT"] = "sheet.mutation.set-worksheet-row-height";
  SheetSkeletonChangeType2["SET_WORKSHEET_ROW_IS_AUTO_HEIGHT"] = "sheet.mutation.set-worksheet-row-is-auto-height";
  SheetSkeletonChangeType2["SET_WORKSHEET_ROW_AUTO_HEIGHT"] = "sheet.mutation.set-worksheet-row-auto-height";
  SheetSkeletonChangeType2["SET_WORKSHEET_COL_WIDTH"] = "sheet.mutation.set-worksheet-col-width";
  SheetSkeletonChangeType2["SET_WORKSHEET_ACTIVE"] = "sheet.operation.set-worksheet-active";
  SheetSkeletonChangeType2["MOVE_ROWS"] = "sheet.mutation.move-rows";
  SheetSkeletonChangeType2["MOVE_COLUMNS"] = "sheet.mutation.move-columns";
  SheetSkeletonChangeType2["SET_COL_HIDDEN"] = "sheet.mutation.set-col-hidden";
  SheetSkeletonChangeType2["SET_COL_VISIBLE"] = "sheet.mutation.set-col-visible";
  SheetSkeletonChangeType2["SET_ROW_HIDDEN"] = "sheet.mutation.set-row-hidden";
  SheetSkeletonChangeType2["SET_ROW_VISIBLE"] = "sheet.mutation.set-row-visible";
  SheetSkeletonChangeType2["INSERT_COL"] = "sheet.mutation.insert-col";
  SheetSkeletonChangeType2["INSERT_ROW"] = "sheet.mutation.insert-row";
  SheetSkeletonChangeType2["REMOVE_COL"] = "sheet.mutation.remove-col";
  SheetSkeletonChangeType2["REMOVE_ROW"] = "sheet.mutation.remove-rows";
  SheetSkeletonChangeType2["TOGGLE_GRIDLINES"] = "sheet.mutation.toggle-gridlines";
  SheetSkeletonChangeType2["SET_GRIDLINES_COLOR"] = "sheet.mutation.set-gridlines-color";
  return SheetSkeletonChangeType2;
}({});
var SheetValueChangeType = function(SheetValueChangeType2) {
  SheetValueChangeType2["SET_RANGE_VALUES"] = "sheet.mutation.set-range-values";
  SheetValueChangeType2["MOVE_RANGE"] = "sheet.mutation.move-range";
  SheetValueChangeType2["REMOVE_WORKSHEET_MERGE"] = "sheet.mutation.remove-worksheet-merge";
  SheetValueChangeType2["ADD_WORKSHEET_MERGE"] = "sheet.mutation.add-worksheet-merge";
  SheetValueChangeType2["REORDER_RANGE"] = "sheet.mutation.reorder-range";
  SheetValueChangeType2["SET_WORKSHEET_DEFAULT_STYLE"] = "sheet.mutation.set-worksheet-default-style";
  SheetValueChangeType2["SET_ROW_DATA"] = "sheet.mutation.set-row-data";
  SheetValueChangeType2["SET_COL_DATA"] = "sheet.mutation.set-col-data";
  SheetValueChangeType2["SET_WORKSHEET_RANGE_THEME_STYLE"] = "sheet.mutation.set-worksheet-range-theme-style";
  SheetValueChangeType2["DELETE_WORKSHEET_RANGE_THEME_STYLE"] = "sheet.mutation.delete-worksheet-range-theme-style";
  return SheetValueChangeType2;
}({});
...
function validateDefinedName(name2, options) {
  if (name2.length === 0)
  ...
  const existingDefinedName = definedNamesService.getValueByName(unitId, name2);
  if (existingDefinedName && (id === null || id === void 0 || id.length === 0 || existingDefinedName.id !== id))
    return "sheets.definedName.nameDuplicate";
  if (superTableService.hasTable(unitId, name2))
    return "sheets.definedName.nameDuplicate";
  if (!Tools.isValidParameter(name2) || isReferenceStringWithEffectiveColumn(name2) || !Tools.isStartValidPosition(name2) && !hasCJKText(name2.substring(0, 1)))
    return "sheets.definedName.nameInvalid";
  const workbook = univerInstanceService.getUnit(unitId, UniverType.UNIVER_SHEET);
  if (!workbook)
    throw new Error(`Workbook not found for unitId: ${unitId}`);
  if (workbook.getSheets().some((sheet) => sheet.getName() === name2))
  ...
  if (functionService.hasExecutor(name2.toUpperCase()))
...
function hasValueFromMatrixWithSpanInfo(cell, matrix) {
  if (cell && cell.spanAnchor)
    return cellHasValue$1(matrix.getValue(cell.spanAnchor.startRow, cell.spanAnchor.startColumn));
...
function getMatrixWithSpanInfo(worksheet, startRow, startColumn, endRow, endColumn) {
  ...
  mergedCellsInRange.forEach((mergedCell) => {
    ...
    createRowColIter(startRow2, endRow2, startColumn2, endColumn2).forEach((row, col) => {
      if (row === startRow2 && col === startColumn2)
...
var createTopMatrixFromRanges = (ranges) => {
...
var createTopMatrixFromMatrix = (matrix) => {
...
var findMaximalRectangle = (topMatrix) => {
  const res = { area: 0 };
...
var filterLeftMatrix = (topMatrix, range) => {
  Range.foreach(range, (row, col) => {
...
var findAllRectangle = (topMatrix) => {
  const resultList = [];
...
var SELECTION_CONTROL_BORDER_BUFFER_WIDTH = 1.5;
var SELECTION_CONTROL_BORDER_BUFFER_COLOR = "rgba(255, 255, 255, 0.01)";
...
var SplitDelimiterEnum = function(SplitDelimiterEnum2) {
  SplitDelimiterEnum2[SplitDelimiterEnum2["Tab"] = 1] = "Tab";
  SplitDelimiterEnum2[SplitDelimiterEnum2["Comma"] = 2] = "Comma";
  SplitDelimiterEnum2[SplitDelimiterEnum2["Semicolon"] = 4] = "Semicolon";
  SplitDelimiterEnum2[SplitDelimiterEnum2["Space"] = 8] = "Space";
  SplitDelimiterEnum2[SplitDelimiterEnum2["Custom"] = 16] = "Custom";
  return SplitDelimiterEnum2;
}({});
...
function getDelimiterRegexItem(delimiter, treatMultipleDelimitersAsOne, customDelimiter) {
  const delimiterList = [];
  if (customDelimiter !== void 0 && (delimiter & 16) > 0)
...
function splitRangeText(sheet, range, delimiter, customDelimiter, treatMultipleDelimitersAsOne = false) {
  ...
  if (delimiter && (delimiter & 16) > 0 && (customDelimiter === void 0 || customDelimiter.length !== 1))
...
var groupByKey = (arr, key, blankKey = "") => {
...
var createUniqueKey = (initValue = 0) => {
...
function findFirstNonEmptyCell(range, worksheet) {
  for (let row = range.startRow; row <= range.endRow; row++)
    for (let col = range.startColumn; col <= range.endColumn; col++)
      if (cellHasValue(worksheet.getCell(row, col)))
        return {
          startRow: row,
          startColumn: col,
          endRow: row,
...
var ViewStateEnum = function(ViewStateEnum2) {
  ViewStateEnum2["OthersCanView"] = "othersCanView";
  ViewStateEnum2["NoOneElseCanView"] = "noOneElseCanView";
  return ViewStateEnum2;
}({});
var EditStateEnum = function(EditStateEnum2) {
  EditStateEnum2["DesignedUserCanEdit"] = "designedUserCanEdit";
  EditStateEnum2["OnlyMe"] = "onlyMe";
  return EditStateEnum2;
}({});
var RangeProtectionRuleModel = class {
  ...
  getSubunitRuleListLength(unitId, subUnitId) {
    ...
    const map2 = (_this$_model$get6 = this._model.get(unitId)) === null || _this$_model$get6 === void 0 ? void 0 : _this$_model$get6.get(subUnitId);
...
var FactoryDeleteRangeProtectionMutation = (accessor, param) => {
...
var FactoryAddRangeProtectionMutation = (param) => {
  const deleteParams = {
    ...param,
...
var SelectionMoveType = function(SelectionMoveType2) {
  SelectionMoveType2[SelectionMoveType2["MOVE_START"] = 0] = "MOVE_START";
  SelectionMoveType2[SelectionMoveType2["MOVING"] = 1] = "MOVING";
  SelectionMoveType2[SelectionMoveType2["MOVE_END"] = 2] = "MOVE_END";
  SelectionMoveType2[SelectionMoveType2["ONLY_SET"] = 3] = "ONLY_SET";
  return SelectionMoveType2;
}({});
...
var SheetsSelectionsService = class SheetsSelectionsService2 extends RxDisposable {
  get _currentSelectionPos() {
    const workbook = this._instanceSrv.getCurrentUnitOfType(UniverType.UNIVER_SHEET);
    if (!workbook)
      return null;
  ...
  _init() {
    const c$ = this._instanceSrv.getCurrentTypeOfUnit$(UniverType.UNIVER_SHEET).pipe(shareReplay({
      bufferSize: 1,
      refCount: true
    ...
    this.disposeWithMe(this._instanceSrv.getTypeOfUnitDisposed$(UniverType.UNIVER_SHEET).pipe(takeUntil(this.dispose$)).subscribe((workbook) => {
  ...
  */
  ...
  setSelections(unitIdOrSelections, worksheetIdOrType, selectionDatas, type) {
    if (typeof unitIdOrSelections === "string" && typeof worksheetIdOrType === "string") {
      ...
      this._ensureWorkbookSelection(unitId2).setSelections(worksheetIdOrType, selectionDatas || [], type !== null && type !== void 0 ? type : 3);
      return;
    }
    const current = this._currentSelectionPos;
    if (!current)
    ...
    if (typeof unitIdOrSelections === "object") {
      ...
      const type2 = (_ref = worksheetIdOrType) !== null && _ref !== void 0 ? _ref : 3;
      this._ensureWorkbookSelection(unitId).setSelections(sheetId, selectionData, type2);
    }
  }
  ...
  */
  getCellStylesProperty(property) {
    ...
    const worksheet = (_this$_instanceSrv$ge = this._instanceSrv.getCurrentUnitOfType(UniverType.UNIVER_SHEET)) === null || _this$_instanceSrv$ge === void 0 ? void 0 : _this$_instanceSrv$ge.getActiveSheet();
    ...
    for (let i = 0; i < selections.length; i++) {
      ...
      for (let row = startRow; row <= endRow; row++)
        for (let column = startColumn; column <= endColumn; column++) {
          ...
          if (value !== void 0 && value !== null && !Tools.diffValue(value, _value))
            return {
              isAllValuesSame: false,
              value: null
            };
...
var DISABLE_NORMAL_SELECTIONS = "DISABLE_NORMAL_SELECTIONS";
var SELECTIONS_ENABLED = "SELECTIONS_ENABLED";
var REF_SELECTIONS_ENABLED = "REF_SELECTIONS_ENABLED";
var SheetSkeletonService = class SheetSkeletonService2 extends Disposable {
  ...
  _init() {
    this.disposeWithMe(this._univerInstanceService.getTypeOfUnitAdded$(UniverType.UNIVER_SHEET).subscribe((event) => this._initWorkbookSkeleton(event.unit)));
    this.disposeWithMe(this._univerInstanceService.getTypeOfUnitDisposed$(UniverType.UNIVER_SHEET).subscribe((workbook) => this._disposeByUnitId(workbook.getUnitId())));
  }
...
function getCellAtRowCol(row, col, worksheet) {
  let destRange = null;
  worksheet.getMatrixWithMergedCells(row, col, row, col).forValue((row2, col2, value) => {
    destRange = {
      actualRow: row2,
      actualColumn: col2,
      startRow: row2,
...
var RefSelectionsService = class RefSelectionsService2 extends SheetsSelectionsService {
  constructor(_instanceSrv) {
    super(_instanceSrv);
  }
  _init() {
  ...
  _getAliveWorkbooks$() {
    const aliveWorkbooks = this._instanceSrv.getAllUnitsForType(UniverType.UNIVER_SHEET);
    ...
    this.disposeWithMe(this._instanceSrv.getTypeOfUnitAdded$(UniverType.UNIVER_SHEET).subscribe((event) => {
      const { unit: workbook } = event;
      this._ensureWorkbookSelection(workbook.getUnitId());
      workbooks$.next([...workbooks$.getValue(), workbook]);
    }));
    this.disposeWithMe(this._instanceSrv.getTypeOfUnitDisposed$(UniverType.UNIVER_SHEET).subscribe((workbook) => {
      this._removeWorkbookSelection(workbook.getUnitId());
...
function getMoveRangeUndoRedoMutations(accessor, from, to, ignoreMerge = false) {
  ...
  const workbook = accessor.get(IUniverInstanceService).getUnit(unitId, UniverType.UNIVER_SHEET);
  if (!workbook)
    return null;
...
var SetRangeValuesCommand = {
  ...
  handler: (accessor, params) => {
    ...
    if (!commandService.syncExecuteCommand(SetRangeValuesMutation.id, setRangeValuesMutationRedoParams))
      return false;
...
var InsertRowByRangeCommand = {
  ...
  handler: (accessor, params) => {
    ...
    if (height !== worksheet.getConfig().defaultRowHeight)
      insertRowParams.rowInfo = new Array(endRow - startRow + 1).fill(void 0).map(() => ({
        h: height,
        hd: BooleanNumber.FALSE
      }));
    const undoRowInsertionParams = InsertRowMutationUndoFactory(accessor, insertRowParams);
...
var InsertColByRangeCommand = {
  ...
  handler: (accessor, params) => {
    ...
    if (width !== worksheet.getConfig().defaultColumnWidth)
      insertColParams.colInfo = new Array(endColumn - startColumn + 1).fill(void 0).map(() => ({
        w: width,
        hd: BooleanNumber.FALSE
      }));
    const undoColInsertionParams = InsertColMutationUndoFactory(accessor, insertColParams);
...
var setSelection = throttle((range, workbook, worksheet, commandService) => {
...
var RemoveColCommand = {
  ...
  handler: async (accessor, params) => {
    ...
    let range = params === null || params === void 0 ? void 0 : params.range;
    if (!range)
      range = (_selectionManagerServ2 = selectionManagerService.getCurrentLastSelection()) === null || _selectionManagerServ2 === void 0 ? void 0 : _selectionManagerServ2.range;
    if (!range)
      return false;
    const target = getSheetCommandTarget(accessor.get(IUniverInstanceService));
    if (!target)
...
var MoveRowsCommand = {
  ...
  handler: (accessor, params) => {
    ...
    if ((filteredSelections === null || filteredSelections === void 0 ? void 0 : filteredSelections.length) !== 1)
      return false;
    const sheetInterceptorService = accessor.get(SheetInterceptorService);
    const target = getSheetCommandTarget(accessor.get(IUniverInstanceService), params);
    if (!target)
...
var OperatorType = function(OperatorType2) {
  OperatorType2[OperatorType2["Set"] = 0] = "Set";
  OperatorType2[OperatorType2["Delete"] = 1] = "Delete";
  OperatorType2[OperatorType2["HorizontalMove"] = 2] = "HorizontalMove";
  OperatorType2[OperatorType2["VerticalMove"] = 3] = "VerticalMove";
  OperatorType2[OperatorType2["Unknown"] = 4] = "Unknown";
...
var handleMoveRowsCommon = (params, targetRange) => {
  const { fromRange, toRange } = params.params || {};
...
var handleReorderRangeCommon = (param, targetRange) => {
...
var handleMoveColsCommon = (params, targetRange) => {
  const { fromRange, toRange } = params.params || {};
  if (!fromRange || !toRange)
    return [targetRange];
...
var handleMoveRange = (param, targetRange) => {
...
var handleMoveRangeCommon = (param, targetRange) => {
...
var handleIRemoveCol = (param, targetRange) => {
  var _param$params5;
...
var handleIRemoveRow = (param, targetRange, rangeFilteredRows) => {
  var _param$params6;
...
var handleReorderRange = (param, targetRange) => {
  const { range, order } = param.params || {};
  if (!range || !order)
    return [];
...
var handleBaseInsertRange = (_insertRange, _targetRange) => {
  ...
  if (targetRange.startColumn < insertRange.startColumn && targetRange.endColumn >= insertRange.startColumn && targetRange.endColumn <= insertRange.endColumn || targetRange.startColumn < insertRange.startColumn && targetRange.endColumn >= insertRange.endColumn)
    return {
      step: 0,
      length: getLength(insertRange)
    };
  if (targetRange.startColumn >= insertRange.startColumn && targetRange.endColumn <= insertRange.endColumn || targetRange.startColumn >= insertRange.startColumn && targetRange.startColumn <= insertRange.endColumn && targetRange.endColumn > insertRange.endColumn || targetRange.startColumn >= insertRange.endColumn)
    return {
...
var handleInsertRow = (param, targetRange) => {
  var _param$params7;
...
var handleInsertCol = (param, targetRange) => {
  var _param$params8;
...
var handleInsertRangeMoveDown = (param, targetRange) => {
  var _param$params9;
...
var handleInsertRangeMoveDownCommon = (param, targetRange) => {
  var _param$params10;
...
var handleInsertRangeMoveRight = (param, targetRange) => {
  var _param$params11;
...
var handleInsertRangeMoveRightCommon = (param, targetRange) => {
  var _param$params12;
...
var handleDeleteRangeMoveLeft = (param, targetRange) => {
  var _param$params13;
...
var handleDeleteRangeMoveLeftCommon = (param, targetRange) => {
  var _param$params14;
...
var handleDeleteRangeMoveUp = (param, targetRange) => {
  var _param$params15;
...
var handleDeleteRangeMoveUpCommon = (param, targetRange) => {
  var _param$params16;
...
var handleRemoveRowCommon = (param, targetRange) => {
  var _param$ranges;
...
var handleInsertRowCommon = (info, targetRange) => {
  ...
  } else if (insertRow <= targetRange.startRow)
    return [{
      ...targetRange,
      startRow: targetRange.startRow + insertCount,
      endRow: targetRange.endRow + insertCount
...
var handleInsertColCommon = (info, targetRange) => {
  const param = info.params;
  ...
  } else if (insertColumn <= targetRange.startColumn)
    return [{
      ...targetRange,
      startColumn: targetRange.startColumn + insertCount,
      endColumn: targetRange.endColumn + insertCount
...
var handleDefaultRangeChangeWithEffectRefCommands = (range, commandInfo) => {
  let operator = [];
  switch (commandInfo.id) {
...
var handleDefaultRangeChangeWithEffectRefCommandsSkipNoInterests = (range, commandInfo, deps) => {
  if ([DeleteRangeMoveLeftCommand.id, DeleteRangeMoveUpCommand.id].includes(commandInfo.id))
    return handleDefaultRangeChangeWithEffectRefCommands(range, commandInfo);
  if (getEffectedRangesOnCommand(commandInfo, deps).some((effectRange) => Rectangle.intersects(effectRange, range)))
    return handleDefaultRangeChangeWithEffectRefCommands(range, commandInfo);
  return range;
};
var handleCommonDefaultRangeChangeWithEffectRefCommands = (range, commandInfo) => {
  let operator = [];
  switch (commandInfo.id) {
    case EffectRefRangId.DeleteRangeMoveLeftCommandId:
    ...
    case EffectRefRangId.InsertColCommandId:
      return handleInsertColCommon(commandInfo, range);
    case EffectRefRangId.InsertRowCommandId:
      return handleInsertRowCommon(commandInfo, range);
    case EffectRefRangId.MoveColsCommandId:
...
var handleCommonRangeChangeWithEffectRefCommandsSkipNoInterests = (range, commandInfo, deps) => {
  if ([
...
var WatchRange = class extends Disposable {
  ...
  onMutation(mutation) {
    ...
    if (((_mutation$params = mutation.params) === null || _mutation$params === void 0 ? void 0 : _mutation$params.unitId) !== this._unitId)
      return;
    if (mutation.id === MoveRangeMutation.id) {
      const params = mutation.params;
      if (params.from.subUnitId !== this._subUnitId || params.to.subUnitId !== this._subUnitId)
        return;
    } else if (((_mutation$params2 = mutation.params) === null || _mutation$params2 === void 0 ? void 0 : _mutation$params2.subUnitId) !== this._subUnitId)
      return;
    if (!this._range)
      return;
    if (this._skipIntersects) {
      ...
      if (effectRanges === null || effectRanges === void 0 ? void 0 : effectRanges.some((effectRange) => Rectangle.intersects(effectRange, this._range)))
        return;
    }
...
var RefRangeService = class RefRangeService2 extends Disposable {
  constructor(_commandService, _sheetInterceptorService, _univerInstanceService, _selectionManagerService) {
    ...
    _defineProperty(this, "_onRefRangeChange", () => {
      this._sheetInterceptorService.interceptCommand({ getMutations: (command) => {
          ...
          switch (command.id) {
            ...
            case EffectRefRangId.MoveRangeCommandId: {
              ...
              if (!sourceTarget && !destinationTarget)
                return [];
              if (sourceTarget && destinationTarget && sourceTarget.unitId === destinationTarget.unitId && sourceTarget.subUnitId === destinationTarget.subUnitId)
                return this._checkRange([params.fromRange, params.toRange], sourceTarget.unitId, sourceTarget.subUnitId);
              const effects = [];
              if (sourceTarget)
                effects.push(...this._checkRange([params.fromRange], sourceTarget.unitId, sourceTarget.subUnitId));
              if (destinationTarget)
                effects.push(...this._checkRange([params.toRange], destinationTarget.unitId, destinationTarget.subUnitId));
              return effects;
            }
            case EffectRefRangId.InsertRowCommandId: {
              const params = command.params;
        ...
        const result = cbList.reduce((result2, currentFn) => {
          ...
          result2.preRedos.push(...(_currentValue$preRedo = currentValue.preRedos) !== null && _currentValue$preRedo !== void 0 ? _currentValue$preRedo : []);
          result2.preUndos.push(...(_currentValue$preUndo = currentValue.preUndos) !== null && _currentValue$preUndo !== void 0 ? _currentValue$preUndo : []);
          return result2;
        }, {
          redos: [],
          undos: [],
...
function getUnitId(univerInstanceService) {
  ...
  return (_univerInstanceServic = univerInstanceService.getCurrentUnitOfType(UniverType.UNIVER_SHEET)) === null || _univerInstanceServic === void 0 ? void 0 : _univerInstanceServic.getUnitId();
}
function getSubUnitId(univerInstanceService) {
  var _univerInstanceServic2;
  return (_univerInstanceServic2 = univerInstanceService.getCurrentUnitOfType(UniverType.UNIVER_SHEET)) === null || _univerInstanceServic2 === void 0 || (_univerInstanceServic2 = _univerInstanceServic2.getActiveSheet()) === null || _univerInstanceServic2 === void 0 ? void 0 : _univerInstanceServic2.getSheetId();
}
...
function createRangeSerializer() {
  ...
  return {
    deserialize: (rangeString) => {
      const map2 = keyList.reduce((preValue, currentValue, index) => {
...
var mutationIdByRowCol$1 = [
  InsertColMutation.id,
  InsertRowMutation.id,
  RemoveColMutation.id,
  RemoveRowMutation.id
];
var mutationIdArrByMove$1 = [MoveRowsMutation.id, MoveColsMutation.id];
...
var MergeCellController = class MergeCellController2 extends Disposable {
    ...
    this._sheetInterceptorService.interceptCommand({ getMutations(commandInfo) {
      switch (commandInfo.id) {
        ...
        case ClearSelectionFormatCommand.id: {
          ...
          const workbook = self._univerInstanceService.getCurrentUnitOfType(UniverType.UNIVER_SHEET);
          const unitId = workbook.getUnitId();
  ...
  _onRefRangeChange() {
    ...
    this._univerInstanceService.getCurrentTypeOfUnit$(UniverType.UNIVER_SHEET).pipe(first((workbook) => !!workbook)).subscribe((workbook) => {
    ...
    this.disposeWithMe(this._commandService.onCommandExecuted((command) => {
      if (mutationIdArrByMove$1.includes(command.id)) {
        ...
        mergeData.forEach((merge2) => {
          ...
          if (!Rectangle.intersects(merge2, sourceRange)) {
            if (isRowMove) {
          ...
          if (!(merge2.startRow === merge2.endRow && merge2.startColumn === merge2.endColumn))
            adjustedMergedCells.push({
              startRow,
              endRow,
              startColumn,
              endColumn,
...
function getWorkbook(univerInstanceService, unitId) {
  ...
  return univerInstanceService.getCurrentUnitOfType(UniverType.UNIVER_SHEET);
}
...
var AddMergeRedoSelectionsOperationFactory = (accessor, params, ranges) => {
  ...
  if (selectionsBeforeMutation && selectionsBeforeMutation.length > 0) {
    ...
    if (primaryBeforeMutation) {
      ...
      const selectionsByRedo = selectionsBeforeMutation.map((selection, index, selections2) => {
        return {
...
var AddMergeUndoSelectionsOperationFactory = (accessor, params) => {
  const selectionsBeforeMutation = accessor.get(SheetsSelectionsService).getCurrentSelections();
  const { unitId, subUnitId } = params;
  if (selectionsBeforeMutation && selectionsBeforeMutation.length > 0) {
...
var RemoveWorksheetMergeCommand = {
  ...
  handler: (accessor, params) => {
    ...
    const intersectsMerges = worksheet.getConfig().mergeData.filter((merge2) => {
      return selections.some((selection) => Rectangle.intersects(selection, merge2));
    });
    if (!intersectsMerges.length)
      return false;
...
function getClearContentMutationParamForRange(worksheet, range) {
  ...
  cellMatrix.forValue((row, col, cellData) => {
    if (cellData && row >= startRow && col >= startColumn) {
      ...
      if (!leftTopCellValue && worksheet.cellHasValue(cellData) && (cellData.v !== "" || ((_cellData$p$body$data = (_cellData$p = cellData.p) === null || _cellData$p === void 0 || (_cellData$p = _cellData$p.body) === null || _cellData$p === void 0 || (_cellData$p = _cellData$p.dataStream) === null || _cellData$p === void 0 ? void 0 : _cellData$p.length) !== null && _cellData$p$body$data !== void 0 ? _cellData$p$body$data : 0) > 2))
...
var AddWorksheetMergeAllCommand = {
  ...
  handler: async (accessor) => {
    ...
    if (!(mergeableSelections === null || mergeableSelections === void 0 ? void 0 : mergeableSelections.length))
      return false;
    const target = getSheetCommandTarget(accessor.get(IUniverInstanceService));
    if (!target)
      return false;
...
function getMergeableSelectionsByType(type, selections) {
  ...
  else if (type === "mergeVertical")
    return selections.filter((selection) => {
      if (selection.startRow === selection.endRow)
        return false;
      return true;
    });
  else if (type === "mergeHorizontal")
    return selections.filter((selection) => {
...
var SetStyleCommand = {
  ...
  handler: (accessor, params) => {
    ...
    if (Tools.isArray(style.value))
      for (let i = 0; i < ranges.length; i++)
        iterator.forOperableEach(ranges[i], (r, c, range2) => {
          cellValue.setValue(r, c, { s: { [style.type]: style.value[r - range2.startRow][c - range2.startColumn] } });
        });
    else
      for (let i = 0; i < ranges.length; i++) {
    ...
    if (autoHeightContext) {
      ...
      const { undos: undos2, redos: redos2 } = interceptor.generateMutationsOfAutoHeight({
        unitId,
        subUnitId,
        ranges: suitableRanges,
        autoHeightRanges: suitableRanges,
      ...
      autoHeightUndos = undos2;
      autoHeightRedos = redos2;
    }
...
function isChnWeek1(txt) {
  let isChnWeek12;
  if (txt.length === 1)
    if (txt === "日" || chineseToNumber$1(txt) < 7)
      isChnWeek12 = true;
    else
      isChnWeek12 = false;
  else
...
function fillChnWeek$1(data, len, step, weekType = 0) {
  ...
  for (let i = 1; i <= len; i++) {
    ...
    removeCellCustom(d);
    let num = 0;
    if (((_data2 = data[data.length - 1]) === null || _data2 === void 0 ? void 0 : _data2.v) === keyword[0])
      num = 7 + step * i;
    else {
      var _data3;
      const last = `${(_data3 = data[data.length - 1]) === null || _data3 === void 0 ? void 0 : _data3.v}`;
      if (last)
        num = chineseToNumber$1(last.substr(last.length - 1, 1)) + step * i;
    }
    if (num < 0)
...
var AUTO_FILL_APPLY_TYPE = function(AUTO_FILL_APPLY_TYPE2) {
  AUTO_FILL_APPLY_TYPE2["COPY"] = "COPY";
  AUTO_FILL_APPLY_TYPE2["SERIES"] = "SERIES";
  AUTO_FILL_APPLY_TYPE2["ONLY_FORMAT"] = "ONLY_FORMAT";
  AUTO_FILL_APPLY_TYPE2["NO_FORMAT"] = "NO_FORMAT";
  return AUTO_FILL_APPLY_TYPE2;
}({});
var AUTO_FILL_DATA_TYPE = function(AUTO_FILL_DATA_TYPE2) {
  AUTO_FILL_DATA_TYPE2["NUMBER"] = "number";
  AUTO_FILL_DATA_TYPE2["DATE"] = "date";
  AUTO_FILL_DATA_TYPE2["EXTEND_NUMBER"] = "extendNumber";
  AUTO_FILL_DATA_TYPE2["CHN_NUMBER"] = "chnNumber";
  AUTO_FILL_DATA_TYPE2["CHN_WEEK2"] = "chnWeek2";
  AUTO_FILL_DATA_TYPE2["CHN_WEEK3"] = "chnWeek3";
  AUTO_FILL_DATA_TYPE2["LOOP_SERIES"] = "loopSeries";
  AUTO_FILL_DATA_TYPE2["FORMULA"] = "formula";
  AUTO_FILL_DATA_TYPE2["OTHER"] = "other";
  return AUTO_FILL_DATA_TYPE2;
}({});
var AUTO_FILL_HOOK_TYPE = function(AUTO_FILL_HOOK_TYPE2) {
  AUTO_FILL_HOOK_TYPE2["APPEND"] = "APPEND";
  AUTO_FILL_HOOK_TYPE2["DEFAULT"] = "DEFAULT";
  AUTO_FILL_HOOK_TYPE2["ONLY"] = "ONLY";
  return AUTO_FILL_HOOK_TYPE2;
}({});
var { chineseToNumber, fillChnNumber, fillChnWeek, fillCopy: fillCopy$1, fillExtendNumber, fillLoopSeries, fillSeries, getLoopSeriesInfo, isChnNumber, isChnWeek2, isChnWeek3, isEqualDiff, isLoopSeries, matchExtendNumber, reverseIfNeed } = AutoFillTools;
var AutoFillRules = {
  dateRule: {
    type: "date",
    priority: 1100,
    match: (cellData, accessor) => {
      if ((cellData === null || cellData === void 0 ? void 0 : cellData.f) || (cellData === null || cellData === void 0 ? void 0 : cellData.si))
        return false;
      if ((typeof (cellData === null || cellData === void 0 ? void 0 : cellData.v) === "number" || (cellData === null || cellData === void 0 ? void 0 : cellData.t) === CellValueType.NUMBER) && cellData.s) {
        if (typeof cellData.s === "string") {
          ...
          if (pattern)
            return lib_exports.getFormatInfo(pattern).isDate;
        } else if (cellData.s.n && lib_exports.getFormatInfo(cellData.s.n.pattern).isDate)
          return true;
      }
      return false;
    },
  ...
  chnNumberRule: {
    ...
    priority: 830,
    match: (cellData) => {
      if (isChnNumber(`${cellData === null || cellData === void 0 ? void 0 : cellData.v}` || ""))
        return true;
      return false;
    },
    isContinue: (prev, cur) => {
      if (prev.type === "chnNumber")
        return true;
      return false;
    },
    applyFunctions: { ["SERIES"]: (dataWithIndex, len, direction) => {
  ...
  chnWeek2Rule: {
    ...
    priority: 820,
    match: (cellData) => {
      if (isChnWeek2(`${cellData === null || cellData === void 0 ? void 0 : cellData.v}` || ""))
        return true;
      return false;
    },
...
var { chnNumberRule, chnWeek2Rule, chnWeek3Rule, dateRule, extendNumberRule, loopSeriesRule, numberRule, otherRule: otherRule$1 } = AutoFillRules;
var AutoFillService = class AutoFillService2 extends Disposable {
  ...
  async triggerAutoFill(unitId, subUnitId, source, selection, manualApplyType) {
    ...
    const first2 = (_this$menu$find = this.menu.find((m) => m.disable === false)) === null || _this$menu$find === void 0 ? void 0 : _this$menu$find.value;
    if (first2)
      return this.fillData(first2);
    return false;
  }
  addHook(hook) {
  ...
  fillData(applyType) {
    ...
    const activeWorkbook = this._univerInstanceService.getCurrentUnitOfType(UniverType.UNIVER_SHEET);
  ...
  _getAutoHeightUndoRedos(executedRedos, workbook, worksheet) {
    ...
    if (!this._commandService.syncExecuteCommand(redo.id, redo.params))
      return {
        undos: [],
        redos: []
      };
...
var configSymbol = Symbol(SHEETS_PLUGIN_CONFIG_KEY);
var defaultLargeSheetOperationConfig = {
  largeSheetCellCountThreshold: 6e3,
  batchSize: 3e3
};
var defaultPluginConfig = {};
var SheetLazyExecuteScheduleService = class SheetLazyExecuteScheduleService2 extends Disposable {
  ...
  */
  ...
  _scheduleNextIdle() {
    ...
    if (typeof requestIdleCallback !== "undefined")
      this._idleCallbackId = requestIdleCallback((deadline) => this._processIdleTasks(deadline), { timeout: 1e3 * 60 });
    else
...
var COPY_SHEET_COMMAND_ID = "sheet.command.copy-sheet";
...
function getCopyUniqueSheetName(workbook, localeService, name2) {
  let output = `${name2} ${localeService.t("sheets.tabs.sheetCopy", "")}`;
  let count = 2;
  while (workbook.checkSheetName(output)) {
    output = `${name2} ${localeService.t("sheets.tabs.sheetCopy", `${count}`)}`;
    count++;
  }
  return output;
}
var DeleteRangeProtectionCommand = {
  ...
  async handler(accessor, params) {
    ...
    if (await commandService.executeCommand(DeleteRangeProtectionMutation.id, redoMutationParam))
      undoRedoService.pushUndoRedo({
        unitID: unitId,
        redoMutations: [{
          id: DeleteRangeProtectionMutation.id,
...
var RegisterWorksheetRangeThemeStyleCommand = {
  ...
  handler: (accessor, params) => {
    ...
    const commandService = accessor.get(ICommandService);
    const undoRedoService = accessor.get(IUndoRedoService);
    if (!getSheetCommandTarget(univerInstanceService, params))
      return false;
    const redoParam = {
      unitId,
    ...
    if (commandService.syncExecuteCommand(RegisterWorksheetRangeThemeStyleMutation.id, redoParam))
      undoRedoService.pushUndoRedo({
        unitID: unitId,
        undoMutations: [{
    ...
    forEach(range, (row, column) => {
      ...
      if (mergedRange) {
        ...
        if (mergedRange.startColumn !== range.startColumn)
          mr.setValue(row, column, { s: { bd: (topLeftStyle === null || topLeftStyle === void 0 ? void 0 : topLeftStyle.bd) ? Object.assign(topLeftStyle.bd, { l: Tools.deepClone(borderStyle) }) : { l: Tools.deepClone(borderStyle) } } });
      } else {
    ...
    forEach(range, (row, column) => {
      ...
      if (mergedRange) {
        ...
        if (mergedRange.startRow !== range.startRow)
          mr.setValue(row, column, { s: { bd: (topLeftStyle === null || topLeftStyle === void 0 ? void 0 : topLeftStyle.bd) ? Object.assign(topLeftStyle.bd, { t: Tools.deepClone(borderStyle) }) : { t: Tools.deepClone(borderStyle) } } });
      } else {
        if (row !== range.endRow) {
          var _mr$getValue7;
...
var SetGridlinesColorCommand = {
  ...
  handler: (accessor, params) => {
    ...
    if (currentlyColor === (params === null || params === void 0 ? void 0 : params.color))
      return false;
    const { unitId, subUnitId } = target;
    const doParams = {
...
var FactorySetRangeProtectionMutation = (accessor, param) => {
...
var SetProtectionCommand = {
  ...
  async handler(accessor, params) {
    ...
    const redoMutations = [];
    const undoMutations = [];
    if ((oldRule === null || oldRule === void 0 ? void 0 : oldRule.unitType) === rule.unitType)
...
var SetRangeCustomMetadataCommand = {
...
var SetWorkbookNameCommand = {
  ...
  handler: (accessor, params) => {
    ...
    const commandService = accessor.get(ICommandService);
    const sheetInterceptorService = accessor.get(SheetInterceptorService);
    if (!getSheetCommandTargetWorkbook(accessor.get(IUniverInstanceService), params))
      return false;
...
var SetWorksheetColIsAutoWidthCommand = {
  ...
  handler: async (accessor, params) => {
    ...
    const { unitId, subUnitId } = target;
    let ranges = [];
    if (params === null || params === void 0 ? void 0 : params.ranges)
      ranges = [...params.ranges];
    else {
...
var SetWorksheetColumnCountCommand = {
  type: CommandType.COMMAND,
...
var SetWorksheetHideCommand = {
  ...
  handler: (accessor, params) => {
    ...
    if (!target)
      return false;
    const { workbook, worksheet, unitId, subUnitId } = target;
    if (worksheet.getConfig().hidden === BooleanNumber.TRUE)
      return false;
    const redoMutationParams = {
      unitId,
      subUnitId,
...
var getAllRangePermissionPoint = () => [
  RangeProtectionPermissionViewPoint,
  RangeProtectionPermissionEditPoint,
  RangeProtectionPermissionManageCollaPoint,
  RangeProtectionPermissionDeleteProtectionPoint
];
var baseProtectionActions = [
  UnitAction.Edit,
  UnitAction.View,
  UnitAction.ManageCollaborator,
  UnitAction.Delete
];
var getDefaultRangePermission = (unitId = "unitId", subUnitId = "subUnitId", permissionId = "permissionId") => getAllRangePermissionPoint().reduce((r, F) => {
...
var WorksheetPermissionService = class WorksheetPermissionService2 extends RxDisposable {
  ...
  _init() {
    ...
    this._univerInstanceService.getAllUnitsForType(UniverType.UNIVER_SHEET).forEach((workbook) => handleWorkbook(workbook));
    this._univerInstanceService.getTypeOfUnitAdded$(UniverType.UNIVER_SHEET).pipe(takeUntil(this.dispose$)).subscribe((event) => handleWorkbook(event.unit));
    this._univerInstanceService.getTypeOfUnitDisposed$(UniverType.UNIVER_SHEET).pipe(takeUntil(this.dispose$)).subscribe((workbook) => {
      workbook.getSheets().forEach((worksheet) => {
        const unitId = workbook.getUnitId();
        const subUnitId = worksheet.getSheetId();
...
var SetWorksheetRowCountCommand = {
  type: CommandType.COMMAND,
...
var SetWorksheetShowCommand = {
  ...
  handler: (accessor, params) => {
    ...
    if (!target)
      return false;
    const { unitId, subUnitId, worksheet } = target;
    if (worksheet.getConfig().hidden === BooleanNumber.FALSE)
      return false;
    const commandService = accessor.get(ICommandService);
    const undoRedoService = accessor.get(IUndoRedoService);
    const redoMutationParams = {
  ...
  handler: (accessor, params) => {
    ...
    for (let i = destRange.startRow; i <= destRange.endRow; i++)
      for (let j = destRange.startColumn; j <= destRange.endColumn; j++) {
        ...
        if (j === 0 && (values === null || values === void 0 ? void 0 : values.length) === 1)
...
var factorySetNumfmtUndoMutation = (accessor, option) => {
...
var factoryRemoveNumfmtUndoMutation = (accessor, option) => {
  const numfmtService = accessor.get(INumfmtService);
...
var transformCellsToRange = (unitId, subUnitId, cells) => {
...
var ToggleGridlinesCommand = {
  ...
  handler: (accessor, params) => {
    ...
    if (currentlyShow === (params === null || params === void 0 ? void 0 : params.showGridlines))
      return false;
    const { unitId, subUnitId } = target;
    const doParams = {
...
var MarkDirtyFilterChangeMutation = {
...
var MarkDirtyRowAutoHeightOperation = {
...
var CancelMarkDirtyRowAutoHeightOperation = {
...
var { getLenS, getDataIndex, fillCopy, fillCopyStyles, generateNullCellValueRowCol } = AutoFillTools;
var { otherRule } = AutoFillRules;
var AutoFillController = class AutoFillController2 extends Disposable {
  ...
  _presetAndCacheData(location, direction) {
    ...
    if (this._shouldDisableSeries(this._copyData)) {
      ...
      this._autoFillService.setDisableApplyType("SERIES", false);
  ...
  _applyFunctions(copySquad, len, direction, applyType, customApplyFunctions, copyDataPiece, location) {
    ...
    if (applyType === "SERIES") {
      ...
      isReverse && data.reverse();
      if (customApplyFunctions === null || customApplyFunctions === void 0 ? void 0 : customApplyFunctions["COPY"])
        return customApplyFunctions["COPY"](copySquad, len, direction, copyDataPiece, location);
      return fillCopy(data, len);
    }
...
var SCOPE_WORKBOOK_VALUE_DEFINED_NAME = "AllDefaultWorkbook";
var DefinedNameDataController = class DefinedNameDataController2 extends Disposable {
...
var sheetsFreezeOnlyLocalMutationIds = [SetFrozenMutation.id];
...
var SheetsFreezeSyncController = class SheetsFreezeSyncController2 extends Disposable {
    ...
    this._d.add(this._commandService.onCommandExecuted((commandInfo, options) => {
      if (effectedByOnlyLocalMutationIds.includes(commandInfo.id) && (options === null || options === void 0 ? void 0 : options.fromCollab)) {
        ...
        if (id === InsertRowMutation.id)
          this._handleInsertRowMutation(params, options);
        else if (id === InsertColMutation.id)
          this._handleInsertColMutation(params, options);
        else if (id === RemoveRowMutation.id)
          this._handleRemoveRowMutation(params, options);
        else if (id === RemoveColMutation.id)
          this._handleRemoveColMutation(params, options);
        else if (id === MoveRowsMutation.id)
          this._handleMoveRowsMutation(params, options);
        else if (id === MoveColsMutation.id)
          this._handleMoveColsMutation(params, options);
      }
    }));
  }
  ...
  _handleMoveRowsMutation(params, options) {
    ...
    const freeze = this._getFreeze(unitId, subUnitId);
    if (!freeze || freeze.startRow <= 0 || sourceRange.startRow >= freeze.startRow && targetRange.startRow >= freeze.startRow || sourceRange.endRow < freeze.startRow && targetRange.endRow < freeze.startRow)
      return;
  ...
  _handleMoveColsMutation(params, options) {
    const { sourceRange, targetRange, unitId, subUnitId } = params;
    const freeze = this._getFreeze(unitId, subUnitId);
    if (!freeze || freeze.startColumn <= 0 || sourceRange.startColumn >= freeze.startColumn && targetRange.startColumn >= freeze.startColumn || sourceRange.endColumn < freeze.startColumn && targetRange.endColumn < freeze.startColumn)
      return;
...
var SheetPermissionCheckController = class SheetPermissionCheckController2 extends Disposable {
  ...
  _commandExecutedListener() {
    ...
    this.disposeWithMe(this._commandService.onCommandExecuted((command) => {
      if (command.id === SetWorksheetNameMutation.id) {
        ...
        if (this._rangeProtectionRuleModel.getSubunitRuleList(unitId, subUnitId).length)
  ...
  _getPermissionCheck(commandInfo) {
    ...
    switch (id) {
      ...
      case RemoveDefinedNameCommand.id:
        params = commandInfo.params;
        if (!params.localSheetId || params.localSheetId === "AllDefaultWorkbook")
          permission = this.permissionCheckWithoutRange({ workbookTypes: [WorkbookEditablePermission] }, params.unitId);
        else
  ...
  permissionCheckWithoutRange(permissionTypes, unitId, subUnitId) {
    ...
    if (rangeTypes && rangeTypes.length > 0) {
      ...
      if (!(permission === null || permission === void 0 ? void 0 : permission.ruleId))
        return true;
    ...
    if (formulaString) {
      ...
      if (definedName) {
        ...
        for (let i = 0; i < refRangesArr.length; i++) {
          ...
          const { sheetName, range: range2 } = deserializeRangeWithSheet(refRange);
          if (sheetName) {
            const targetSheet = workbook.getSheetBySheetName(sheetName);
            if (!targetSheet)
              return true;
            const { startRow, endRow, startColumn, endColumn } = range2;
            for (let r = startRow; r <= endRow; r++)
              for (let c = startColumn; c <= endColumn; c++) {
                ...
                if ((permission === null || permission === void 0 ? void 0 : permission[UnitAction.View]) === false)
                  return false;
              }
          }
        }
        ...
        for (let i = 0; i < sequenceNodes.length; i++) {
          ...
          if (typeof node === "string" || node.nodeType !== sequenceNodeType.REFERENCE)
            continue;
          const { token } = node;
          const { unitId: sequenceGridUnitId, sheetName, range: range2 } = deserializeRangeWithSheetWithCache(token);
          let sequenceGridWorkbook = workbook;
          if (sequenceGridUnitId && sequenceGridUnitId !== unitId) {
            const _sequenceGridWorkbook = this._univerInstanceService.getUnit(sequenceGridUnitId, UniverType.UNIVER_SHEET);
            if (!_sequenceGridWorkbook)
              return true;
          ...
          if (sheetName) {
            ...
            if (!viewPermission || viewPermission.value === false)
              return false;
          }
          const { startRow, endRow, startColumn, endColumn } = range2;
          for (let r = startRow; r <= endRow; r++)
  ...
  _permissionCheckBySetRangeValue(permissionTypes, params) {
    ...
    if ((Tools.isArray(value) || isICellData(value)) && range)
      ...
      ranges = [new ObjectMatrix(value).getStartEndScope()];
  ...
  _permissionCheckByMoveRangeCommand(params) {
    ...
    if (!this.permissionCheckWithRanges({
      ...
      worksheetTypes: [WorksheetSetCellValuePermission, WorksheetEditPermission],
      rangeTypes: [RangeProtectionPermissionEditPoint]
    }, [fromRange], fromTarget.unitId, fromTarget.subUnitId))
      return false;
    return this.permissionCheckWithRanges({
      workbookTypes: [WorkbookEditablePermission],
      worksheetTypes: [WorksheetSetCellValuePermission, WorksheetEditPermission],
  ...
  _permissionCheckWithInsertOrDeleteMoveRange(direction, params) {
    ...
    if (rules.some((rule) => rule.ranges.some((range2) => Rectangle.intersects(range2, _range))))
      return false;
    return true;
  }
};
...
var WorkbookPermissionService = class WorkbookPermissionService2 extends Disposable {
  ...
  _init() {
    ...
    this._univerInstanceService.getAllUnitsForType(UniverType.UNIVER_SHEET).forEach((workbook) => handleWorkbook(workbook));
    this.disposeWithMe(this._univerInstanceService.getTypeOfUnitAdded$(UniverType.UNIVER_SHEET).subscribe((event) => handleWorkbook(event.unit)));
    this.disposeWithMe(this._univerInstanceService.getTypeOfUnitDisposed$(UniverType.UNIVER_SHEET).subscribe((workbook) => {
      const unitId = workbook.getUnitId();
      workbook.getSheets().forEach((worksheet) => {
        const subUnitId = worksheet.getSheetId();
        this._rangeProtectionRuleModel.getSubunitRuleList(unitId, subUnitId).forEach((rule) => {
...
var SheetPermissionInitController = class SheetPermissionInitController2 extends Disposable {
  ...
  async _initRangePermissionFromSnapshot() {
    const initRangePermissionFunc = async (workbook) => {
      ...
      this._authzIoService.batchAllowed(allAllowedParams).then((permissionMap) => {
        permissionMap.forEach((item) => {
          ...
          if (rule) {
            if (!this._rangeProtectionRuleModel.getRule(unitId, rule.subUnitId, rule.id))
              return;
            getAllRangePermissionPoint().forEach((F) => {
              ...
              if ((result === null || result === void 0 ? void 0 : result.allowed) !== void 0)
    ...
    await Promise.all(this._univerInstanceService.getAllUnitsForType(UniverType.UNIVER_SHEET).map((workbook) => initRangePermissionFunc(workbook)));
    this._rangeProtectionRuleModel.changeRuleInitState(true);
  }
    ...
    this.disposeWithMe(this._rangeProtectionRuleModel.ruleChange$.subscribe((info) => {
      if (info.type !== "delete")
        this._authzIoService.allowed({
          ...
          if (!this._rangeProtectionRuleModel.getRule(info.unitId, info.subUnitId, info.rule.id))
            return;
          getAllRangePermissionPoint().forEach((F) => {
            if (info.type === "set") {
              const { rule: rule2, oldRule } = info;
              if (rule2.permissionId === (oldRule === null || oldRule === void 0 ? void 0 : oldRule.permissionId))
                return;
            }
  ...
  async initWorkbookPermissionChange(_unitId) {
    var _this$_univerInstance;
    const unitId = _unitId || ((_this$_univerInstance = this._univerInstanceService.getCurrentUnitOfType(UniverType.UNIVER_SHEET)) === null || _this$_univerInstance === void 0 ? void 0 : _this$_univerInstance.getUnitId());
    if (!unitId)
      return;
  ...
  async _initWorkbookPermissionFromSnapshot() {
    await Promise.all(this._univerInstanceService.getAllUnitsForType(UniverType.UNIVER_SHEET).map((workbook) => this.initWorkbookPermissionChange(workbook.getUnitId())));
  ...
  async _initWorksheetPermissionFromSnapshot() {
    ...
    await Promise.all(this._univerInstanceService.getAllUnitsForType(UniverType.UNIVER_SHEET).map((workbook) => initSheetPermissionFunc(workbook)));
    this._worksheetProtectionRuleModel.changeRuleInitState(true);
  }
  _initUserChange() {
  ...
  refreshPermission(unitId, permissionId) {
    ...
    if (sheetRuleItem) {
      ...
      this._authzIoService.allowed({
        ...
        if (!this._worksheetProtectionRuleModel.getTargetByPermissionId(unitId, permissionId))
          return;
        let key = "";
        getAllWorksheetPermissionPoint().forEach((F) => {
          const instance = new F(unitId, subUnitId);
          ...
          if (action) {
            var _this$_permissionServ;
            if (((_this$_permissionServ = this._permissionService.getPermissionPoint(instance.id)) === null || _this$_permissionServ === void 0 ? void 0 : _this$_permissionServ.value) !== action.allowed)
    ...
    if (sheetPointItem) {
      ...
      this._authzIoService.allowed({
        ...
        if (!this._worksheetProtectionPointRuleModel.getTargetByPermissionId(unitId, permissionId))
          return;
        getAllWorksheetPermissionPointByPointPanel().forEach((F) => {
          const instance = new F(unitId, subUnitId);
          const unitActionName = instance.subType;
          ...
          if (action) {
            var _this$_permissionServ2;
            if (((_this$_permissionServ2 = this._permissionService.getPermissionPoint(instance.id)) === null || _this$_permissionServ2 === void 0 ? void 0 : _this$_permissionServ2.value) !== action.allowed)
              needClearUndoRedo = true;
            this._permissionService.updatePermissionPoint(instance.id, action.allowed);
          }
        });
    ...
    if (rangeRuleItem) {
      ...
      this._authzIoService.allowed({
        ...
        if (!this._rangeProtectionRuleModel.getTargetByPermissionId(unitId, permissionId))
          return;
        let key = "";
        getAllRangePermissionPoint().forEach((F) => {
          ...
          if (action) {
            var _this$_permissionServ3;
            if (((_this$_permissionServ3 = this._permissionService.getPermissionPoint(instance.id)) === null || _this$_permissionServ3 === void 0 ? void 0 : _this$_permissionServ3.value) !== action.allowed)
              needClearUndoRedo = true;
            this._permissionService.updatePermissionPoint(instance.id, action.allowed);
            key += `${action.action}_${action.allowed}`;
          }
...
var ZebraCrossingCacheController = class ZebraCrossingCacheController2 extends Disposable {
...
var RangeProtectionRenderModel = class RangeProtectionRenderModel2 {
  ...
  _init() {
    this._permissionService.permissionPointUpdate$.pipe(filter((permission) => permission.type === UnitObject.SelectRange), filter((permission) => getAllRangePermissionPoint().some((F) => permission instanceof F)), map((permission) => permission)).subscribe((permission) => {
    ...
    for (const rule of ruleMap)
      if (rule.ranges.some((range) => range.startRow <= row && range.endRow >= row && range.startColumn <= col && range.endColumn >= col)) {
        const permissionMap = getAllRangePermissionPoint().reduce((result2, F) => {
          ...
          result2[instance.subType] = (_permission$value = permission === null || permission === void 0 ? void 0 : permission.value) !== null && _permission$value !== void 0 ? _permission$value : instance.value;
          return result2;
        }, {});
        result.push({
          ...permissionMap,
...
var RangeProtectionCache = class RangeProtectionCache2 extends Disposable {
...
var version = "0.25.0";
var ActiveWorksheetController = class ActiveWorksheetController2 extends Disposable {
  constructor(_commandService, _univerInstanceService) {
    super();
    this._commandService = _commandService;
    this._univerInstanceService = _univerInstanceService;
    ...
    this.disposeWithMe(this._commandService.beforeCommandExecuted((command) => {
      if (command.id === RemoveSheetMutation.id)
        return this._beforeAdjustActiveSheetOnRemoveSheet(command);
    }));
    this.disposeWithMe(this._commandService.onCommandExecuted((command, options) => {
      if (command.id === RemoveSheetMutation.id)
  ...
  _adjustActiveSheetOnHideSheet(mutation) {
    ...
    if (!workbook)
      return;
    if (((_workbook$getActiveSh = workbook.getActiveSheet()) === null || _workbook$getActiveSh === void 0 ? void 0 : _workbook$getActiveSh.getSheetId()) !== subUnitId)
      return;
  ...
  _adjustActiveSheetOnSelection(operation) {
    ...
    if (subUnitId !== ((_this$_univerInstance = this._univerInstanceService.getUnit(unitId)) === null || _this$_univerInstance === void 0 ? void 0 : _this$_univerInstance.getActiveSheet().getSheetId()))
      this._switchToNextSheet(unitId, subUnitId);
  }
...
var BasicWorksheetController = class BasicWorksheetController2 extends Disposable {
...
var CalculateResultApplyController = class CalculateResultApplyController2 extends Disposable {
...
var NumberCellDisplayController = class NumberCellDisplayController2 extends Disposable {
  ...
  _initInterceptorCellContent() {
    this.disposeWithMe(this._sheetInterceptorService.intercept(INTERCEPTOR_POINT.CELL_CONTENT, {
      ...
      handler: (cell, location, next) => {
        ...
        if (!isDefaultFormat(style === null || style === void 0 || (_style$n = style.n) === null || _style$n === void 0 ? void 0 : _style$n.pattern))
          return next(cell);
...
var SheetPermissionViewModelController = class SheetPermissionViewModelController2 extends Disposable {
...
var NumfmtService = class NumfmtService2 extends Disposable {
  ...
  getValue(unitId, subUnitId, row, col) {
    ...
    if (cell === null || cell === void 0 ? void 0 : cell.s) {
      ...
      if (style === null || style === void 0 ? void 0 : style.n)
        return style.n;
    }
    return null;
  }
...
var RangeProtectionRefRangeService = class RangeProtectionRefRangeService2 extends Disposable {
  ...
  _onRefRangeChange() {
    const registerRefRange = (unitId, subUnitId) => {
      const workbook2 = this._univerInstanceService.getCurrentUnitOfType(UniverType.UNIVER_SHEET);
      if (!workbook2)
        return;
      if (!(workbook2 === null || workbook2 === void 0 ? void 0 : workbook2.getSheetBySheetId(subUnitId)))
        return;
      this.disposableCollection.dispose();
      const handler = (config) => {
        return this.refRangeHandle(config, unitId, subUnitId);
...
var PLUGIN_NAME = "SHEET_RANGE_PROTECTION_PLUGIN";
var RangeProtectionService = class RangeProtectionService2 extends Disposable {
...
var SheetRangeThemeService = class SheetRangeThemeService2 extends Disposable {
...
var UniverSheetsPlugin = class UniverSheetsPlugin2 extends Plugin {
  ...
  _initConfig() {
    ...
    if ((_this$_config = this._config) === null || _this$_config === void 0 ? void 0 : _this$_config.onlyRegisterFormulaRelatedMutations)
      this._configService.setConfig(ONLY_REGISTER_FORMULA_RELATED_MUTATIONS_KEY, true);
    if ((_this$_config2 = this._config) === null || _this$_config2 === void 0 ? void 0 : _this$_config2.isRowStylePrecedeColumnStyle)
      this._configService.setConfig(IS_ROW_STYLE_PRECEDE_COLUMN_STYLE, true);
    if ((_this$_config3 = this._config) === null || _this$_config3 === void 0 ? void 0 : _this$_config3.autoHeightForMergedCells)
  ...
  _initDependencies() {
    ...
    if (!((_this$_config4 = this._config) === null || _this$_config4 === void 0 ? void 0 : _this$_config4.notExecuteFormula))
  ...
  onStarting() {
    ...
    if (!((_this$_config5 = this._config) === null || _this$_config5 === void 0 ? void 0 : _this$_config5.onlyRegisterFormulaRelatedMutations))
...
_defineProperty(UniverSheetsPlugin, "type", UniverType.UNIVER_SHEET);
UniverSheetsPlugin = __decorate([
  DependentOn(UniverFormulaEnginePlugin),
  __decorateParam(1, Inject(Injector)),
  __decorateParam(2, IConfigService)
], UniverSheetsPlugin);
var PermissionPointsDefinitions = {
  /**
...
var checkRangesEditablePermission = (accessor, unitId, subUnitId, ranges) => {
  ...
  if (!(workbookEditablePermission === null || workbookEditablePermission === void 0 ? void 0 : workbookEditablePermission.value))
    return false;
  const worksheetEditPermission = permissionService.getPermissionPoint(new WorksheetEditPermission(unitId, subUnitId).id);
  if (!(worksheetEditPermission === null || worksheetEditPermission === void 0 ? void 0 : worksheetEditPermission.value))
    return false;
...
var getPrimaryCellUp = (scopeRange, worksheet, currentPrimary, step = 1, isFindNext = true, isGoBack = true) => {
  ...
  if (next >= startRow)
    ...
    return getPrimaryCellLeft(scopeRange, worksheet, {
      ...currentPrimary,
      startRow: endRow,
      endRow
    }, step, isFindNext, false);
};
var getPrimaryCellDown = (scopeRange, worksheet, currentPrimary, step = 1, isFindNext = true, isGoBack = true) => {
  const { startRow, endRow } = Range.transformRange(scopeRange, worksheet);
  ...
  if (next <= endRow)
    ...
    return getPrimaryCellRight(scopeRange, worksheet, {
      ...currentPrimary,
      startRow,
      endRow: startRow
    }, step, isFindNext, false);
};
var getPrimaryCellLeft = (scopeRange, worksheet, currentPrimary, step = 1, isFindNext = true, isGoBack = true) => {
  ...
  if (next >= startColumn)
    ...
    return getPrimaryCellUp(scopeRange, worksheet, {
      ...currentPrimary,
      startColumn: endColumn,
      endColumn
    }, step, isFindNext, false);
};
var getPrimaryCellRight = (scopeRange, worksheet, currentPrimary, step = 1, isFindNext = true, isGoBack = true) => {
  const { startColumn, endColumn } = Range.transformRange(scopeRange, worksheet);
  ...
  if (next <= endColumn)
    ...
    return getPrimaryCellDown(scopeRange, worksheet, {
      ...currentPrimary,
      startColumn,
      endColumn: startColumn
    }, step, isFindNext, false);
...
var getPrimaryCell = (scopeRange, worksheet, currentPrimary, direction, step = 1) => {
  switch (direction) {
    case Direction.UP:
...
var getNextPrimaryCell = (selections, direction, worksheet) => {
  let activeSelection;
  let activeIndex = -1;
  let currentPrimary;
  for (let i = 0; i < selections.length; i++)
  ...
  if (!Rectangle.equals(activeSelection.range, primary)) {
    ...
    if (!next)
      return null;
    const destRange2 = useLeftTopAsDest ? getCellAtRowCol$1(next.endRow, next.endColumn, worksheet) : getCellAtRowCol$1(next.startRow, next.startColumn, worksheet);
    return {
      startRow: destRange2.startRow,
      startColumn: destRange2.startColumn,
      endRow: destRange2.endRow,
      endColumn: destRange2.endColumn
    };
  }
...
//# sourceMappingURL=chunk-3LU3DNZ5.js.map
