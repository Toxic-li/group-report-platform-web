...
import "./chunk-RWNGK3AZ.js";
import "./chunk-JWQJNSGI.js";
import {
  AbsoluteRefType,
  BehaviorSubject,
  BooleanNumber,
    ...
    list.forEach((item) => {
      ...
      for (let i = startRow; i <= endRow; i++)
        for (let j = startColumn; j <= endColumn; j++)
          if (i !== row || j !== column)
            cellMatrix.setValue(i, j, { si: formulaId });
    });
    ...
    if (!Rectangle.equals(lastRow, targetRange)) {
      for (const cell of worksheet.iterateByColumn(lastRow))
        if (!cell.value || !worksheet.cellHasValue(cell.value))
          setValueMatrix.setValue(cell.row, cell.col, { f: `=SUM(${serializeRange({
...
var CalculationMode = function(CalculationMode2) {
...
var FormulaAutoFillController = class FormulaAutoFillController2 extends Disposable {
...
var FormulaCalculationSessionController = class FormulaCalculationSessionController2 extends Disposable {
  ...
  _initialize() {
    this.disposeWithMe(this._commandService.onCommandExecuted((command, options) => {
      ...
      if (command.id === SetRangeValuesMutation.id && (options === null || options === void 0 ? void 0 : options.applyFormulaCalculationResult))
...
var ImageFormulaCellInterceptorController = class ImageFormulaCellInterceptorController2 extends Disposable {
...
var TriggerCalculationController = class TriggerCalculationController2 extends Disposable {
  _emitProgress(label) {
    this._progress$.next({
  ...
  constructor(_commandService, _univerInstanceService, _activeDirtyManagerService, _logService, _configService, _formulaDataModel, _localeService, _registerOtherFormulaService) {
    ...
    this.disposeWithMe(this._univerInstanceService.getTypeOfUnitAdded$(UniverType.UNIVER_SHEET).subscribe(() => {
      this._initialExecuteFormula();
    }));
  }
  dispose() {
  ...
  _commandExecutedListener() {
    this.disposeWithMe(this._commandService.beforeCommandExecuted((command) => {
      if (command.id === SetFormulaCalculationStartMutation.id || command.id === SetFormulaStringBatchCalculationMutation.id) {
        const params = command.params;
        if (command.id === SetFormulaCalculationStartMutation.id)
          params.isCalculateTreeModel = this._configService.getConfig(ENGINE_FORMULA_RETURN_DEPENDENCY_TREE) || false;
  ...
  _mergeDirtyUnitStringMap(allDirtyMap, dirtyMap) {
    Object.keys(dirtyMap).forEach((unitId) => {
      ...
      Object.keys(dirtyMap[unitId]).forEach((dirtyKey) => {
        ...
        if ((_dirtyMap$unitId = dirtyMap[unitId]) === null || _dirtyMap$unitId === void 0 ? void 0 : _dirtyMap$unitId[dirtyKey])
...
function checkFormulaDataNull(formulaData, unitId, sheetId) {
  ...
  if (formulaData == null || formulaData[unitId] == null || ((_formulaData$unitId = formulaData[unitId]) === null || _formulaData$unitId === void 0 ? void 0 : _formulaData$unitId[sheetId]) == null)
    return true;
  return false;
}
...
function getFormulaReferenceMoveUndoRedo(oldFormulaData, newFormulaData, formulaReferenceMoveParam) {
  ...
  if (formulaReferenceSheetList.includes(type) || type === 17 && formulaReferenceMoveParam.range == null)
...
function processFormulaChanges(oldFormulaMatrix, newFormulaMatrix, formulaReferenceMoveParam) {
  ...
  oldFormulaMatrix.forValue((row, column, cell) => {
    if (cell == null || !isFormulaDataItem(cell))
      return true;
...
function handleMove(type, from, to, oldCell) {
  ...
  switch (type) {
    case 0:
      return handleRefMoveRange$1(from, to, oldCell);
    case 1:
      return handleRefMoveRows$1(from, to, oldCell);
    case 2:
      return handleRefMoveCols$1(from, to, oldCell);
    default:
      return null;
  }
}
...
function getRedoFormulaData(rangeList, oldFormulaMatrix, newFormulaMatrix) {
  ...
  for (let i = 0; i < rangeList.length; i++) {
    ...
    if (!(((_redoFormulaData$getV = redoFormulaData.getValue(oldCell.startRow, oldCell.startColumn)) === null || _redoFormulaData$getV === void 0 ? void 0 : _redoFormulaData$getV.f) || ((_redoFormulaData$getV2 = redoFormulaData.getValue(oldCell.startRow, oldCell.startColumn)) === null || _redoFormulaData$getV2 === void 0 ? void 0 : _redoFormulaData$getV2.si)))
      redoFormulaData.setValue(oldCell.startRow, oldCell.startColumn, {
        f: null,
        si: null
      });
    if (newCell) {
...
function getNewRangeByMoveParam(unitRangeWidthOffset, formulaReferenceMoveParam, currentFormulaUnitId, currentFormulaSheetId, options = {}) {
  ...
  if (!checkIsSameUnitAndSheet(userUnitId, userSheetId, currentFormulaUnitId, currentFormulaSheetId, sequenceRangeUnitId, sequenceRangeSheetId))
    return;
...
function handleRefSetWorksheetName(command, workbook) {
  ...
  if (!params)
    return null;
  const { unitId, subUnitId, name: name2 } = params;
...
function handleRefSetDefinedName(command, workbook) {
  ...
  if (!params)
    return null;
  const { unitId, name: name2, id } = params;
...
function handleRefSetSheetTableName(command, workbook) {
  const { params } = command;
  if (!params || !params.name || !params.oldTableName || params.oldTableName === params.name)
    return null;
...
function handleRefRemoveSheetTableColumn(command, workbook) {
  ...
  const { params } = command;
  if (!params || !params.tableName || !((_params$removedColumn = params.removedColumnNames) === null || _params$removedColumn === void 0 ? void 0 : _params$removedColumn.length))
    return null;
...
var UpdateDefinedNameController = class UpdateDefinedNameController2 extends Disposable {
    ...
    Object.values(definedNames).forEach((item) => {
      ...
      for (let i = 0, len = sequenceNodes.length; i < len; i++) {
        ...
        if (type === 12)
          ...
          newRefString = getNewRangeByMoveParam(sequenceUnitRangeWidthOffset, moveParams, unitId, sheetId, { preserveSheetQualifier: true });
  ...
  _removeSheet(item, unitId, subUnitId) {
    ...
    const { formulaOrRefString } = item;
    if (((_this$_definedNamesSe = this._definedNamesService.getWorksheetByRef(unitId, formulaOrRefString)) === null || _this$_definedNamesSe === void 0 ? void 0 : _this$_definedNamesSe.getSheetId()) === subUnitId)
      return ErrorType.REF;
    return null;
  }
};
...
var UpdateFormulaController = class UpdateFormulaController2 extends Disposable {
  ...
  _commandExecutedListener() {
    ...
    this.disposeWithMe(this._univerInstanceService.getTypeOfUnitAdded$(UniverType.UNIVER_SHEET).subscribe((event) => this._handleWorkbookAdded(event.unit)));
    this.disposeWithMe(this._univerInstanceService.getTypeOfUnitDisposed$(UniverType.UNIVER_SHEET).pipe(map((unit) => unit.getUnitId())).subscribe((unitId) => this._handleWorkbookDisposed(unitId)));
  }
        ...
        matrixData.forValue((row, column, formulaDataItem) => {
          ...
          for (let i = 0, len = sequenceNodes.length; i < len; i++) {
            ...
            if ((type === 13 || type === 14) && (nodeType === sequenceNodeType.DEFINED_NAME || nodeType === sequenceNodeType.FUNCTION)) {
              ...
              if (oldTableName === void 0 || type === 15 && tableName === void 0)
                continue;
              const { tableName: tokenTableName, columnStruct = "" } = splitTableStructuredRef(token);
              if (tokenTableName !== oldTableName)
                continue;
              if (type === 17 && !tableReferenceContainsColumn(columnStruct, tableColumnNames))
                continue;
              sequenceNodes[i] = {
                ...node,
              ...
              if ((type === 16 || type === 17) && typeof nextNode === "string" && nextNode.startsWith("]"))
            ...
            if (!checkIsSameUnitAndSheet(formulaReferenceMoveParam.unitId, formulaReferenceMoveParam.sheetId, unitId, sheetId, sequenceUnitId, sequenceSheetId))
              continue;
            const sequenceUnitRangeWidthOffset = {
              range,
              sheetId: sequenceSheetId,
            ...
            if (type === 11) {
              ...
              newRefString = getNewRangeByMoveParam(sequenceUnitRangeWidthOffset, formulaReferenceMoveParam, unitId, sheetId, { inCrossSheetCutRange });
            if (newRefString != null) {
              sequenceNodes[i] = {
                ...node,
                token: newRefString
              ...
              shouldModify = true;
              refChangeIds.push(i);
              if (si && (x !== null && x !== void 0 ? x : 0) === 0 && (y !== null && y !== void 0 ? y : 0) === 0)
                shouldModifySi.push(si);
            }
          ...
          if (!shouldModify)
            if (si && [
              ...
              if (from && from.startRow <= row && row <= from.endRow && from.startColumn <= column && column <= from.endColumn) {
                if ((x !== null && x !== void 0 ? x : 0) === 0 && (y !== null && y !== void 0 ? y : 0) === 0)
                  shouldModifySi.push(si);
...
function tableReferenceContainsColumn(columnStruct, columnNames) {
  if (!(columnNames === null || columnNames === void 0 ? void 0 : columnNames.length) || columnStruct.length === 0)
    return false;
...
var ActiveDirtyController = class ActiveDirtyController2 extends Disposable {
...
var ArrayFormulaCellInterceptorController = class ArrayFormulaCellInterceptorController2 extends Disposable {
    ...
    Object.entries(dataModel).forEach(([unitId, subUnitData]) => {
      subUnitData && Array.from(Object.entries(subUnitData)).forEach(([subUnitId, formulaDataItem]) => {
        ...
        new ObjectMatrix(formulaDataItem).forValue((row, col, value) => {
          ...
          if ((value === null || value === void 0 ? void 0 : value.x) != null || !functionText || functionText.length === 0)
            return;
    ...
    arrayFormulaEmbedded && Object.entries(arrayFormulaEmbedded).forEach(([unitId, subUnitData]) => {
      subUnitData && Array.from(Object.entries(subUnitData)).forEach(([subUnitId, rangeData]) => {
        ...
        new ObjectMatrix(rangeData).forValue((row, col) => {
          var _arrayFormulaRange$un;
          if (arrayFormulaRange === null || arrayFormulaRange === void 0 || (_arrayFormulaRange$un = arrayFormulaRange[unitId]) === null || _arrayFormulaRange$un === void 0 || (_arrayFormulaRange$un = _arrayFormulaRange$un[subUnitId]) === null || _arrayFormulaRange$un === void 0 || (_arrayFormulaRange$un = _arrayFormulaRange$un[row]) === null || _arrayFormulaRange$un === void 0 ? void 0 : _arrayFormulaRange$un[col])
            return;
...
var DescriptionService = class DescriptionService2 extends Disposable {
  ...
  unregisterDescriptions(functionNames) {
    ...
    functionNames.forEach((name2) => {
      const functionName = name2.toUpperCase();
  ...
  hasDescription(name2) {
    return this._descriptions.has(name2.toUpperCase());
  }
  hasDefinedNameDescription(name2) {
    const item = this._descriptions.get(name2.toUpperCase());
    if (!item)
      return false;
    return item.functionType === FunctionType.DefinedName;
  }
  isFormulaDefinedName(name2) {
    const item = this._descriptions.get(name2.toUpperCase());
    if (!item)
      return false;
...
var DefinedNameController = class DefinedNameController2 extends Disposable {
  ...
  _changeUnitListener() {
    this.disposeWithMe(toDisposable(this._univerInstanceService.getCurrentTypeOfUnit$(UniverType.UNIVER_SHEET).subscribe((workbook) => {
  ...
  _updateDescriptions(event) {
    ...
    if (type === "update") {
      ...
      definedNames.forEach((definedName) => {
        const { name: name2, comment, formulaOrRefString, localSheetId } = definedName;
  ...
  _registerDescription(params) {
    ...
    const { name: name2, comment, formulaOrRefString, localSheetId } = params;
    if (this._descriptionService.hasDescription(name2))
      return;
    if (localSheetId == null || localSheetId === SCOPE_WORKBOOK_VALUE_DEFINED_NAME || localSheetId === subUnitId)
  ...
  _unregisterDescription(param) {
    const { name: name2 } = param;
    this._descriptionService.unregisterDescriptions([name2]);
  }
  ...
  _initRegisterDescriptions(unitId, subUnitId) {
    ...
    Array.from(Object.values(definedNames)).forEach((value) => {
      const { name: name2, comment, formulaOrRefString, localSheetId } = value;
      if (this._descriptionService.hasDescription(name2))
        return;
  ...
  _unregisterDescriptionsForNotInSheetId(unitId, subUnitId) {
    ...
    const functionList = [];
    Array.from(Object.values(definedNames)).forEach((value) => {
      const { name: name2, localSheetId } = value;
...
var FormulaController = class FormulaController2 extends Disposable {
...
var SuperTableController = class SuperTableController2 extends Disposable {
  ...
  _changeUnitListener() {
    toDisposable(this._univerInstanceService.getCurrentTypeOfUnit$(UniverType.UNIVER_SHEET).subscribe((workbook) => {
      this._unRegisterDescriptions();
      if (workbook)
        this._registerDescriptions();
    }));
...
var FormulaRefRangeService = class FormulaRefRangeService2 extends Disposable {
  ...
  transformFormulaByEffectCommand(unitId, subUnitId, formula, params) {
    ...
    const currentUnit = this._univerInstanceService.getCurrentUnitOfType(UniverType.UNIVER_SHEET);
      ...
      for (const { unitId: depUnitId, subUnitId: depSubUnitId, ranges } of deps)
        if (depUnitId === effectedRanges.unitId && depSubUnitId === effectedRanges.subUnitId) {
          ...
          if (intersectedRanges.length > 0)
            matchedEffectedRanges.push(intersectedRanges.map((range) => ({
...
var RemoteRegisterFunctionService = class RemoteRegisterFunctionService2 {
  constructor(_functionService) {
    this._functionService = _functionService;
  }
  async registerFunctions(serializedFuncs) {
    const functionList = serializedFuncs.map(([func, name2]) => {
      return createFunction(func, name2);
    });
    this._functionService.registerExecutors(...functionList);
  }
  async registerAsyncFunctions(serializedFuncs) {
    const functionList = serializedFuncs.map(([func, name2]) => {
      return createAsyncFunction(func, name2);
    });
    this._functionService.registerExecutors(...functionList);
  }
...
var IRegisterFunctionService = createIdentifier("sheets-formula.register-function-service");
var RegisterFunctionService = class RegisterFunctionService2 extends Disposable {
  ...
  _registerSingleFunction(params) {
    const { name: name2, func, description, locales, async = false } = params;
    const disposables = new DisposableCollection();
    if (locales)
      this._localeService.load(locales);
    ...
    const instance = async ? new AsyncCustomFunction(name2) : new CustomFunction(name2);
    instance.calculateCustom = func;
    this._functionService.registerExecutors(instance);
    disposables.add(toDisposable(() => this._functionService.unregisterExecutors(name2)));
    disposables.add(toDisposable(() => this._functionService.unregisterDescriptions(name2)));
    disposables.add(toDisposable(() => this._functionService.deleteFormulaAstCacheKey(name2)));
    if (this._remoteRegisterFunctionService) {
      this._remoteRegisterFunctionService.registerAsyncFunctions([[func.toString(), name2]]);
      disposables.add(toDisposable(() => this._remoteRegisterFunctionService.unregisterFunctions([name2])));
    }
    return disposables;
  }
  _registerLocalExecutors(list) {
    const names = list.map(([_func, name2]) => name2);
    const functions = list.map(([func, name2]) => {
      const instance = new CustomFunction(name2);
      instance.calculateCustom = func;
      return instance;
    });
  ...
  _registerRemoteExecutors(list) {
    ...
    const functions = list.map(([func, name2]) => {
      functionNameList.push(name2);
      return [func.toString(), name2];
    });
...
var UniverRemoteSheetsFormulaPlugin = class UniverRemoteSheetsFormulaPlugin2 extends Plugin {
  constructor(_config = defaultPluginRemoteConfig, _injector, _configService) {
    super();
    this._config = _config;
    this._injector = _injector;
    this._configService = _configService;
    const { ...rest } = merge_default({}, defaultPluginRemoteConfig, this._config);
...
_defineProperty(UniverRemoteSheetsFormulaPlugin, "type", UniverType.UNIVER_SHEET);
UniverRemoteSheetsFormulaPlugin = __decorate([
  DependentOn(UniverFormulaEnginePlugin),
  __decorateParam(1, Inject(Injector)),
  __decorateParam(2, IConfigService)
], UniverRemoteSheetsFormulaPlugin);
var UniverSheetsFormulaPlugin = class UniverSheetsFormulaPlugin2 extends Plugin {
  constructor(_config = defaultPluginBaseConfig, _injector, _configService) {
    super();
    this._config = _config;
    this._injector = _injector;
    this._configService = _configService;
    const { ...rest } = merge_default({}, defaultPluginBaseConfig, this._config);
  ...
  onReady() {
    ...
    if (isNodeEnv())
      touchDependencies(this._injector, [[TriggerCalculationController], [FormulaCalculationSessionController]]);
  }
  onRendered() {
...
_defineProperty(UniverSheetsFormulaPlugin, "type", UniverType.UNIVER_SHEET);
...
//# sourceMappingURL=@univerjs_sheets-formula.js.map
